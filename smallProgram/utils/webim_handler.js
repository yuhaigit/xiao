...
function showMsg(msg) {
    ...
    var isSelfSend, fromAccount, fromAccountNick, sessType, subType, sendTime,headUrl,isTime;
    ...
    subType = msg.getSubType();
    if(subType == 2){  //进群提示
      fromAccountNick = msg.elems[0].content.userinfo[0].NickName
    }
    isSelfSend = msg.getIsSend();//消息是否为自己发的
    headUrl = msg.fromAccountHeadurl;
  if (Number(msg.time) - Number(showTime)<=300){
      isTime = false;
    }else{
      isTime = true;
    }
...
function formatTime(ts) {   //时间格式转换
  let timeStamp = new Date(new Date().setHours(0, 0, 0, 0)) / 1000;
  let thisdate;
  let d = new Date(ts * 1000); //根据时间戳生成的时间对象
  ...
  if (timeStamp > ts) {
    thisdate = ((month + 1) + '月' + day + '日 ' + hour + ":" + min);
  } else {
    thisdate = (hour + ":" + min);
  }
  return thisdate
